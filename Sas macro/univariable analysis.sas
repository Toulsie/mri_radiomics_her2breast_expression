 ...
 %if &clist_cnt > 0  %then %do; 
   ...
   *NUMERIC Variables;
   ...
   DATA _report;
      ...
      /* OR and 95% CI */
      if OddsRatio ~= . then 
         OR_CI = TRIM(LEFT(PUT(OddsRatio,8.1))) || " (" || 
         TRIM(LEFT(PUT(lowerCL,8.1))) || ", " || 
         TRIM(LEFT(PUT(upperCL,8.1))) || ")";
      else OR_CI = '-';
   RUN;
   ...
   **** PRINT THE TABLE ************;
   ...
   PROC REPORT DATA=_report HEADLINE HEADSKIP CENTER STYLE(REPORT)={JUST=CENTER} SPLIT='~'
     nowd SPANROWS LS=256 ; 
      ...
      %IF &clist_cnt > 0 %then %do; 
         DEFINE Level/order order=Data  DISPLAY   "Level"     STYLE(COLUMN) = {JUST =L}; 
      %end;
      DEFINE Frequency/DISPLAY "N" STYLE(COLUMN) = {JUST = C};                
      DEFINE OR_CI/DISPLAY "Odds Ratio~(95% CI)" STYLE(COLUMN) = {JUST = C CellWidth=15%}; 
      DEFINE ProbChiSq/DISPLAY  "OR P-value" STYLE(COLUMN) = {JUST = C CellWidth=8%} FORMAT=LTPVAL.; 
