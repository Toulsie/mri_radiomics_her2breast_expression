...
model her2 (ref="0")= T1_original_glszm_ZoneEntropy 
T2_original_firstorder_10Percent 
T2_original_firstorder_Energy 
T2_original_glcm_Imc1 
T2_original_gldm_SmallDepen_0002 
T2_original_ngtdm_Strength 
T2_original_glcm_JointAverage /rsquare outroc=rocdata2;
 output out=preds pred=p;
format her2 he. cat_infiltrant cat. ;
run;
ods graphics off;

*ods  pdf dpi=300  file="C:\Users\toulsie\Desktop\F1metrics.pdf" startpage=never ; options orientation=portrait;
%prcurve(data=rocdata2, inpred=preds, pred=p, options=optimal , optvars=pred )
ods pdf close;
